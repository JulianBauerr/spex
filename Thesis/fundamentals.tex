...
\begin{itemize}
    \item \textbf{Ladder Operators ($\hat{\sigma}_k^\pm$):} Defined using the standard Pauli matrices as $\hat{\sigma}_k^\pm = \frac{1}{2}(\hat{\sigma}_x \pm i\hat{\sigma}_y)$. These operators execute the local ``creation'' or ``annihilation'' at the specific target qubit $k$ by raising or lowering the computational basis state between $\ket{0}$ (empty) and $\ket{1}$ (occupied).
    \item \textbf{Parity Strings ($\hat{\sigma}_z^{\otimes (N-k)}$):} A non-local tensor product of Pauli-$Z$ operators acting on the adjacent qubits. This string encodes the necessary parity information into the phase of the wavefunction.
\end{itemize}
